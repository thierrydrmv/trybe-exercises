...
VALUES ('Cinemark', 'São Paulo'), (
        'Brodway theater',
        'Nova York'
    ), ('Phoenix theater', 'Londres'), ('Le Champo', 'Paris'), (
        'TLC Chinese Theater',
        'Los Angeles'
...
VALUES (1, 8.3, 190000000, 170000000), (2, 7.2, 160000000, 200600000), (3, 7.9, 245000000, 239000000), (4, 6.1, 330000000, 540000000), (5, 7.8, 140000000, 310000000), (6, 5.8, 540000000, 600000000), (7, 7.5, 250000000, 190000000), (8, 8.5, 300000000, 250000000), (10, 7.4, 460000000, 510000000), (9, 6.8, 450000000, 370000000), (11, 9.9, 290000000, 280000000);
...
FROM pixar.movies m
    INNER JOIN pixar.box_office s ON m.id = s.movie_id;
...
FROM pixar.movies m
    INNER JOIN pixar.box_office s ON m.id = s.movie_id
WHERE
    s.domestic_sales < s.international_sales;

-- 🚀 Exercício 3: Utilizando o INNER JOIN, faça uma busca que retorne os filmes e sua avaliação (rating) em ordem decrescente.

SELECT m.title, s.rating
FROM pixar.movies m
    INNER JOIN pixar.box_office s
...
SELECT m.*, r.rating
FROM pixar.movies m
    INNER JOIN pixar.box_office r ON m.id = movie_id
WHERE
    r.rating > 8
